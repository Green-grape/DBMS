...
buffer_t* using_buffer(int table_id,pagenum_t pagenum,int mode){
	...
	pthread_mutex_lock(&(temp->page_mutex));
	pthread_mutex_unlock(&(buffer_manager_mutex));
	return temp;
}

int buf_read_page_with_lock_record(int table_id, pagenum_t pagenum, page_t* dest, int trx_id, int idx){
	buffer_t* buffer=using_buffer(table_id,pagenum,READ);
	if(buffer!=NULL){
		(*dest)=buffer->frame;
		lock_t* ret_lock=NULL;
		int res=lock_acquire(table_id, pagenum, trx_id, READ, idx, &ret_lock);
		if(res==ACQUIRED){
			pthread_mutex_unlock(&(buffer->page_mutex));
			return 0; //ok
		}
		else if(res==NEED_TO_WAIT){
			pthread_mutex_unlock(&(buffer->page_mutex));
			lock_wait(ret_lock);
			buffer=using_buffer(table_id,pagenum,READ);
			pthread_mutex_unlock(&(buffer->page_mutex));
			return 0;
		}else if(res==DEAD_LOCK){
			pthread_mutex_unlock(&(buffer->page_mutex));
			trx_trx_abort(trx_id);
			return -2;
		}
	}
	return -1;
...
int buf_write_page_with_lock_record(int table_id, pagenum_t pagenum, const page_t* src, int trx_id, int idx, char* back_up){
	buffer_t* buffer=using_buffer(table_id,pagenum,WRITE);
	if(buffer!=NULL){
		buffer->frame=(*src);
		buffer->isDirty=true;
		lock_t* ret_lock=NULL;
		int res=lock_acquire(table_id, pagenum, trx_id, WRITE, idx, &ret_lock);
		trx_insert_backup(table_id, pagenum, idx, back_up,trx_id);
		if(res==ACQUIRED){
			pthread_mutex_unlock(&(buffer->page_mutex));
			return 0; //ok
		}
...
int64_t buf_read_root_page(int table_id, page_t* dest){
	...
	if(buffer!=NULL){
		if(buffer->frame.number_of_page==1){//no root page
			pthread_mutex_unlock(&(buffer->page_mutex));
			return -1;
		}
...
int buf_write_root_page(int table_id, pagenum_t pagenum, const page_t* src){
	...
	if(buffer!=NULL){
		...
		// 	buffer_header->head->isPinned=false;
		// }
		int res=buf_write_page(table_id,pagenum,src);
		return res;
	}
	return -1;
}
