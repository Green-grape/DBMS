...
int tree_height[MAX_TABLE_ID+1]={0,};
...
int open_table(char* pathname){
    ...
    buf_read_page(filepathNum,0 ,header_pages[filepathNum]);
...
int64_t db_find_leaf(int table_id, int64_t key, int trx_id) {
    ...
    int64_t c=buf_read_root_page(table_id, (page_t*)cur_page);
    ...
    while (!cur_page->isLeaf) {
       ...
       if(i==-1){
            ...
            if(buf_read_page(table_id, res,(page_t*)cur_page)==-2) return -2;
       		if(error_check==true){
       			//printf("find_leaf\n");
       			res=-1;
       			break;
...
int64_t db_find_leaf_page(int table_id, int64_t key, internal_page_t* cur_page, int caller) {
    //pthread_mutex_lock(&update_mutex);
    int i = 0;
    int64_t c=buf_read_root_page(table_id, (page_t*)cur_page);

    if(c<=0){
        printf("no root page, error");
        free(cur_page);
        return c;
    } 
    ...
    while (!cur_page->isLeaf) {
       ...
       if(i==-1){
            res=cur_page->left_most_page_num;
                //printf("page:%ld mode:%d\n", res, 0);
            if(buf_read_page(table_id, res,(page_t*)cur_page)==-2){
                    return -2;
            }
            if(error_check==true){
                //printf("find_leaf\n");
...
int db_find(int table_id, int64_t key, char* ret_val, int trx_id) {
    ...
    int64_t leaf_page_num =db_find_leaf_page(table_id, key, (internal_page_t*)leaf_page,0);
    if(leaf_page_num < 1) {
        if(leaf_page_num==-2){
    ...
    if(i==leaf_page->number_of_keys) {
        ...
        if(trx_id!=0) res=buf_read_page_with_lock_record(table_id,leaf_page_num,(page_t*)leaf_page, trx_id, i);
        if(res==-2){
            printf("trx_id: %d buf_read_page_with_lock abort!\n");
        }
    	else{
...
int db_update(int table_id, int64_t key, char* values, int trx_id){
    ...
    leaf_page_t* leaf_page=(leaf_page_t*)malloc(sizeof(leaf_page_t));
    memset(leaf_page,0,sizeof(leaf_page_t));
    int64_t leaf_page_num =db_find_leaf_page(table_id, key, (internal_page_t*)leaf_page,1);
    if(leaf_page_num < 1) {
        if(leaf_page_num==-2){
            printf("trx_id: %d db_find_leaf abort!\n", trx_id);
            return -2;
    ...
    if(error_check==false){
        ...
        if(i==leaf_page->number_of_keys) {
            ...
            char saved_value[120];
            strcpy(saved_value, leaf_page->key_value[i].value);
            strcpy(leaf_page->key_value[i].value, values);
            res=buf_write_page_with_lock_record(table_id, leaf_page_num, (page_t*)leaf_page, trx_id, i,saved_value);
            if(res==-2){
...
int db_roll_back(transaction_back_up_t* trx_back_up){
    ...
    leaf_page_t* leaf_page=(leaf_page_t*)malloc(sizeof(leaf_page_t));
    memset(leaf_page,0,sizeof(leaf_page_t));
    int64_t leaf_page_num =db_find_leaf_page(trx_back_up->table_id, trx_back_up->pagenum, (internal_page_t*)leaf_page,0);
    if(leaf_page_num < 1) {
        return -1;
    }
    if(error_check==false){
        for(i=0;i<leaf_page->number_of_keys;i++){
            if(leaf_page->key_value[i].key==trx_back_up->idx) break;
        }
        if(i==leaf_page->number_of_keys) {
            //printf("no key\n");
            res=-1;
            ...
            strcpy(leaf_page->key_value[i].value, trx_back_up->back_up);
            res=buf_write_page(trx_back_up->table_id, leaf_page_num, (page_t*)leaf_page);
        }
    }
    else res=-1;
    free(leaf_page);
...
int db_insert_into_leaf(int table_id, int64_t leaf_page_num, leaf_page_t* leaf_page, int64_t key, char *value) {
    ...
    buf_write_page(table_id, leaf_page_num,(page_t*)leaf_page);
...
int db_insert_into_leaf_after_splitting(int table_id, int64_t leaf_page_num, leaf_page_t* leaf_page, int64_t key, char* value) {
    ...
    if(new_leaf_page_num>0){
    	buf_write_page(table_id, leaf_page_num,(page_t*)leaf_page);
    	if(error_check==false){
    		buf_write_page(table_id, new_leaf_page_num,(page_t*)new_leaf_page);
    		free(new_leaf_page);
            new_leaf_page=NULL;
    		if(error_check==false) {
...
int db_insert_into_internal_after_splitting(int table_id, pagenum_t old_page_num, pagenum_t right_page_num, int64_t key, int left_index, internal_page_t* old_page) {
    ...
    for(i=0;i<split-1;i++){
        ...
        if(i==split-2){
            ...
            buf_read_page(table_id, temp_pair[i].page_num, (page_t*)t_internal);
            if(t_internal->isLeaf){
                ((leaf_page_t*)t_internal)->right_sibling_page_num=0;
                buf_write_page(table_id, temp_pair[i].page_num,(page_t*)t_internal);
            }
            free(t_internal);
        }
    }
    if(left_index+1<split-1){
        buf_read_page(table_id, right_page_num,(page_t*)child_page);
        child_page->parent_page_num=old_page_num;
        buf_write_page(table_id, right_page_num,(page_t*)child_page);
    }
    ...
    buf_read_page(table_id, new_internal->left_most_page_num, (page_t*)child_page);
    child_page->parent_page_num=new_internal_page_num;
    buf_write_page(table_id, new_internal->left_most_page_num,(page_t*)child_page);
    for(++i,j=0;i<=MAX_KEY_INTL;i++,j++){
    	new_internal->key_page[j]=temp_pair[i];
    	new_internal->number_of_keys++;
        buf_read_page(table_id, new_internal->key_page[j].page_num, (page_t*)child_page);
        child_page->parent_page_num=new_internal_page_num;
        buf_write_page(table_id, new_internal->key_page[j].page_num, (page_t*)child_page);
    }
    free(child_page);
    ...
    if(new_internal_page_num>0){
    	buf_write_page(table_id, new_internal_page_num,(page_t*)new_internal);
    	if(error_check==false){
    		buf_write_page(table_id, old_page_num,(page_t*)old_page);
    		if(error_check==false){
                internal_page_t* right_page=(internal_page_t*)malloc(sizeof(internal_page_t));
                memset(right_page,0,sizeof(internal_page_t));
                buf_read_page(table_id, right_page_num,(page_t*)right_page);
...
int db_insert_into_new_root(int table_id, pagenum_t left_page_num, pagenum_t right_page_num, int64_t key) {
    ...
    buf_read_page(table_id, left_page_num,(page_t*)left_page);
    if(error_check==false){
    	buf_read_page(table_id, right_page_num,(page_t*)right_page);
    	if(error_check==false){
            ...
            buf_write_page(table_id,new_root_page_num,(page_t*)new_root_page);
            buf_set_root_page_num(table_id, new_root_page_num);
    		if(error_check==false){
    			buf_write_page(table_id, right_page_num,(page_t*)right_page);
    			if(error_check==false){
                    buf_write_page(table_id, left_page_num,(page_t*)left_page);
                    free(left_page);
                    left_page=NULL;
                    free(right_page);
                    right_page=NULL;
...
int db_start_new_tree(int table_id, int64_t key, char* value) {
    ...
    new_root_page->number_of_keys++;
    buf_write_root_page(table_id, buf_get_root_page_num(table_id),(page_t*)new_root_page);
    free(new_root_page);
    if(error_check==false) return 0;
    else return -1;
}
...
int db_insert(int table_id, int64_t key, char* value) {
    ...
    int c=buf_read_page(table_id, 0, header_page);
    ...
    if(error_check==false)
    {
    	if (header_page->number_of_page==1){
    		...
    		if(leaf_page_num>0){
                ...
                memset(leaf_page,0,sizeof(leaf_page_t));
    			buf_read_page(table_id, leaf_page_num,(page_t*)leaf_page);
    			if(error_check==false){
...
int db_set_right_sibling_insert(int table_id, int64_t parent_page_num, int64_t child_page_num){
    ...
    if(parent_page->left_most_page_num!=child_page_num && i<parent_page->number_of_keys && i>0){
        buf_read_page(table_id, parent_page->key_page[i-1].page_num,(page_t*)before_child);
        buf_read_page(table_id, child_page_num,(page_t*)child);
        if(error_check==false){
            child->right_sibling_page_num=before_child->right_sibling_page_num;
            before_child->right_sibling_page_num=child_page_num;
            buf_write_page(table_id, parent_page->key_page[i-1].page_num,(page_t*)before_child);
            if(error_check==false) res=1;
            else res=-1;
        }else res=-1;
    }
    else if(i==0){
        buf_read_page(table_id, parent_page->left_most_page_num,(page_t*)before_child);
        buf_read_page(table_id, child_page_num,(page_t*)child);
        if(error_check==false){
            child->right_sibling_page_num=before_child->right_sibling_page_num;
            before_child->right_sibling_page_num=child_page_num;
            buf_write_page(table_id, parent_page->left_most_page_num,(page_t*)before_child);
            if(error_check==false) res=1;
            else res=-1;
        }else res=-1;
    }
...
void print_page(int table_id){
    ...
    for(int64_t i=0;i<num_of_pages;i++){
        buf_read_page(table_id, i,(page_t*)cur_page);
        printf("pagenum:%ld\n",i);
        if(i==0){
...
int db_delete_entry(int table_id, int64_t leaf_page_num,int64_t key) {
   ...
   //printf("in delete entry");
     ...
     */
    if (leaf_page_num==buf_get_root_page_num(table_id)){
    ...
    if(error_check==false){
        if(parent_page->number_of_keys==1){
            if(parent_page->parent_page_num==0){
                ...
                memset(new_root_page,0,sizeof(internal_page_t));
                if(parent_page->left_most_page_num==child_page_num){
                    buf_read_page(table_id, parent_page->key_page[0].page_num,(page_t*)new_root_page);
                    new_root_page->parent_page_num=0;
                    buf_write_page(table_id, parent_page->key_page[0].page_num,(page_t*)new_root_page);
                    buf_set_root_page_num(table_id, parent_page->key_page[0].page_num);
                }
                else if(parent_page->key_page[0].page_num==child_page_num){
                    buf_read_page(table_id, parent_page->left_most_page_num,(page_t*)new_root_page);
                    new_root_page->parent_page_num=0;
                    buf_write_page(table_id, parent_page->left_most_page_num,(page_t*)new_root_page);
            ...
            buf_read_page(table_id, p_parent_page_num,(page_t*)p_parent_page);
            ...
            buf_read_page(table_id, p_parent_page->key_page[neighbor_index].page_num,(page_t*)neighbor_index_page);
            ...
            if(neighbor_index_page->number_of_keys==MAX_KEY_INTL){//neighbor is full: split and Insert
                ...
                if(deletion_index<neighbor_index){
                    ...
                    for(i=0;i<split;i++){
                        ...
                        parent_page->number_of_keys++;
                       	buf_read_page(table_id, parent_page->key_page[i].page_num, (page_t*)child_page);
                        child_page->parent_page_num=parent_page_num;
                       	buf_write_page(table_id, parent_page->key_page[i].page_num, (page_t*)child_page);
                    }
                    ...
                    buf_read_page(table_id, parent_page->left_most_page_num, (page_t*)child_page);
                    child_page->parent_page_num=parent_page_num;
                    buf_write_page(table_id, parent_page->left_most_page_num,(page_t*)child_page);
                ...
                buf_write_page(table_id, p_parent_page->key_page[deletion_index].page_num,(page_t*)parent_page);
                if(error_check==false){
                    buf_write_page(table_id, p_parent_page->key_page[neighbor_index].page_num,(page_t*)neighbor_index_page);
                    if(error_check==false){
                        buf_write_page(table_id, p_parent_page_num,(page_t*)p_parent_page);
                        if(error_check==false){
                            res=0;
                        }
                    }
                ...
                p_parent_page=NULL;
                    ...
                    if(deletion_index<neighbor_index){
                        ...
                        if(child_page_num==parent_page->left_most_page_num) neighbor_index_page->left_most_page_num=parent_page->key_page[0].page_num;
                       	...
                       	buf_read_page(table_id, neighbor_index_page->left_most_page_num,(page_t*)child_page);
                        child_page->parent_page_num=p_parent_page->key_page[neighbor_index].page_num;
                        buf_write_page(table_id, neighbor_index_page->left_most_page_num,(page_t*)child_page);
                        ...
                        buf_read_page(table_id, neighbor_index_page->key_page[neighbor_index_page->number_of_keys].page_num,(page_t*)child_page);
                        child_page->parent_page_num=p_parent_page->key_page[neighbor_index].page_num;
                        buf_write_page(table_id, neighbor_index_page->key_page[neighbor_index_page->number_of_keys].page_num,(page_t*)child_page);
                        neighbor_index_page->number_of_keys++;
...
int db_set_right_sibling_delete(int table_id, int64_t parent_page_num, int64_t child_page_num){
    ...
    memset(before_child,0,sizeof(leaf_page_t));
    memset(child,0,sizeof(leaf_page_t));
    if(0<i && i<parent_page->number_of_keys) buf_read_page(table_id, parent_page->key_page[i-1].page_num,(page_t*)before_child);
    else if(i==0) buf_read_page(table_id, parent_page->left_most_page_num,(page_t*)before_child);
    else{
        free(before_child);
        free(child);
        free(parent_page);
    ...
    buf_read_page(table_id, parent_page->key_page[i].page_num,(page_t*)child);
    before_child->right_sibling_page_num=child->right_sibling_page_num;
    if(0<i && i<parent_page->number_of_keys) buf_write_page(table_id, parent_page->key_page[i-1].page_num,(page_t*)before_child);
    else if(i==0) buf_write_page(table_id, parent_page->left_most_page_num,(page_t*)before_child);
    free(before_child);
    free(child);
    free(parent_page);
    return 0;
