...
int64_t db_find_leaf(int table_id, int64_t key) {
    ...
    int c=buf_read_root_page(table_id, (page_t*)cur_page);
    ...
    int64_t res=buf_get_root_page_num(table_id);
    ...
    while (!cur_page->isLeaf) {
       ...
       if(i==-1){
       		...
       		buf_read_page(table_id, res,(page_t*)cur_page);
       		if(error_check==true){
       			//printf("find_leaf\n");
       			res=-1;
       			break;
...
int db_find(int table_id, int64_t key, char* ret_val) {
    ...
    int64_t leaf_page_num =db_find_leaf(table_id, key);
    if(leaf_page_num < 1) return -1;
...
int db_insert(int table_id, int64_t key, char* value) {
    ...
    if (db_find(table_id, key, value)==0) return -1;
